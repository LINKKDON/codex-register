...
def fetch_sentinel_challenge(session, device_id, flow="authorize_continue", user_agent=None, sec_ch_ua=None, impersonate=None, proxy_url=None):
...
def build_sentinel_token(session, device_id, flow="authorize_continue", user_agent=None, sec_ch_ua=None, impersonate=None, proxy_url=None):
    """构建完整的 openai-sentinel-token JSON 字符串"""
    challenge = fetch_sentinel_challenge(session, device_id, flow=flow, user_agent=user_agent, sec_ch_ua=sec_ch_ua, impersonate=impersonate, proxy_url=proxy_url)
...
def generate_datadog_trace():
    ...
    span_id = random.randint(1, 2**63 - 1)
    return {
